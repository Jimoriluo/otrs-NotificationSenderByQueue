...
# ---
    ...
    my %QueueMappingAddress = %{ $ConfigObject->Get('NotificationSenderByQueue::MappingAddress') || {} };
    my %QueueMappingName    = %{ $ConfigObject->Get('NotificationSenderByQueue::MappingName') || {} };

    my $QueueSender;
    my $QueueName;

    my $Queue = $Ticket{Queue};
    if ( $QueueMappingAddress{$Queue} ) {
        $QueueSender = $QueueMappingAddress{$Queue};
        $QueueName   = $QueueSender;

        if ( $QueueMappingName{$Queue} ) {
            $QueueName = sprintf "%s <%s>", $QueueMappingName{$Queue}, $QueueSender;
        }
    }
# ---
